...
export async function startBridge(options: { port?: number; token?: string; pairingCode?: string; authorizationPath?: string } = {}) {
  ...
  const connections = new Map<string, { grant: string; instance: string; sessionId: string }>();
  function connect(c: Context, grant: string, instance: unknown) {
    ...
    for (const [credential, connection] of connections) {
      if (!live.has(connection.sessionId)) { connections.delete(credential); pluginTokens.delete(credential); continue; }
      // A retried resume belongs to one running plugin, never to a document name.
      if (connection.grant === grant && connection.instance === instance) {
        broker.touch(connection.sessionId, c);
        return { ok: true, sessionId: connection.sessionId, token: credential, protocol: VERSION };
      }
    }
    const sessionId = broker.register(c); const credential = randomBytes(32).toString('hex');
    pluginTokens.set(credential, sessionId); connections.set(credential, { grant, instance, sessionId });
  ...
  const server = createServer((req, res) => { void (async () => {
    ...
    if (['/resume', '/forget'].includes(url.pathname) && req.method === 'POST') {
      ...
      if (url.pathname === '/resume') return send(res, 200, connect(context(input.context), grant, input.instanceId));
      await authorizations.revoke(grant);
      for (const [credential, connection] of connections) if (connection.grant === grant) { broker.disconnect(connection.sessionId); pluginTokens.delete(credential); connections.delete(credential); }
    ...
    if (url.pathname === '/pair' && req.method === 'POST') {
      ...
      if (input.instanceId !== undefined && (typeof input.instanceId !== 'string' || !/^[a-zA-Z0-9-]{1,100}$/.test(input.instanceId))) throw new AgentError('INVALID_INSTANCE', 'Invalid plugin instance ID.');
      pinExpiry = 0;
      const resumeToken = await authorizations.issue();
      return send(res, 200, { ...connect(c, authorizations.identify(resumeToken)!, input.instanceId ?? randomBytes(16).toString('hex')), resumeToken });
    }
    ...
    if (url.pathname.startsWith('/plugin/')) {
      ...
      if (url.pathname === '/plugin/disconnect') { broker.disconnect(sessionId); pluginTokens.delete(bearer); connections.delete(bearer); return send(res, 200, { ok: true }); }
    } else {
