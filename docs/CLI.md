...
单文件命令串行执行。暂停接收会阻止后续派发，点击暂停前已经派发的命令仍会执行。多个文件的队列互相独立。断开或关闭插件会结束当前会话，重新打开会使用已保存的授权自动连接。切换文件共用设备授权，每个文件仍需运行插件；CLI 以独立会话路由，不按文件名合并。异常关闭时旧会话最多约 65 秒后过期。取消此设备绑定会撤销授权并断开所有使用它的会话。Figma 本机存储被清除、插件 ID 改变或服务端授权文件丢失时，需要重新绑定。
